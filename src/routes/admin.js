...
router.post('/tasks', adminOnly, async (req, res) => {
  try {
    const { type, title, reward, icon, link } = req.body
    if (!title || !reward) return res.status(400).json({ error: 'title and reward required' })
    const { rows: [task] } = await pool.query(
      'INSERT INTO tasks (type, title, reward, icon, link, active) VALUES ($1,$2,$3,$4,$5,true) RETURNING *',
      [type || 'custom', title, reward, icon || '⭐', link || null]
    )
    res.json(task)
  } catch (e) {
    res.status(500).json({ error: e.message })
